...
export default function generateFilter(
  includeDomains = [],
  includeEntities = [],
  excludeDomains = [],
  excludeEntities = []
  ...
  if (!haveInclude && haveExclude) {
    return entityId => (!excludeEntities.has(entityId) &&
  ...
  if (includeDomains.size) {
    return entityId =>
      (includeDomains.has(computeDomain(entityId)) ?
        !excludeEntities.has(entityId) : includeEntities.has(entityId));
  }
  ...
  if (excludeDomains.size) {
    return entityId =>
      (excludeDomains.has(computeDomain(entityId)) ?
        includeEntities.has(entityId) : !excludeEntities.has(entityId));
  }
